...
static struct button_info_st button_info[] = { { "func", IT87_DEB0 },
					       { "display", IT87_DEB1 } };
...
struct fan_info_st fan_info[] = {
	{ IT87_EC_FAN_TACO1, IT87_EC_FAN_TACO1_EXT, IT87_EC_FAN_SMART_PWM1,
	  IT87_EC_FAN_PWM1, 0x1 },
	{ IT87_EC_FAN_TACO2, IT87_EC_FAN_TACO2_EXT, IT87_EC_FAN_SMART_PWM2,
	  IT87_EC_FAN_PWM2, 0x2 },
};
...
/* Device ID offset */
unsigned char device_id_offset[32] = { 0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b, 0x4c,
				       0x4d, 0x4e, 0x4f, 0x50, 0x51, 0x52, 0x53,
				       0x54, 0x55, 0x56, 0x57, 0x59, 0x5a, 0x5b,
				       0x5c, 0x5d, 0x5e, 0x5f, 0x66, 0x67, 0x68,
				       0x69, 0x6a, 0x6b, 0x6c };
...
{
	return single_open(file, show_button_status,
			   PDE_DATA(file_inode(file)));
}

#if HAVE_PROC_OPS
static struct proc_ops button_status_fops = { .proc_open = open_button_status,
					      .proc_read = seq_read,
					      .proc_lseek = seq_lseek,
					      .proc_release = single_release };
#else
static struct file_operations button_status_fops = { .owner = THIS_MODULE,
...
static unsigned long fan_speed_ul_to_str(char *buf, size_t len,
					 unsigned long speed)
{
...
static ssize_t show_temp2(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	return sprintf(buf, "%d\n", get_temp(1));
}

static ssize_t show_temp3(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	return sprintf(buf, "%d\n", get_temp(2));
}
...
static ssize_t show_fan1_rpm(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	return sprintf(buf, "%d\n", get_fan_rpm(0));
}

static ssize_t show_fan2_rpm(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	return sprintf(buf, "%d\n", get_fan_rpm(1));
}
...
/* ----------- speed attributes ---------- */
static ssize_t show_fan1_speed(struct device *dev,
...
static ssize_t store_fan1_speed(struct device *dev,
...
static ssize_t show_fan2_speed(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	if (flag_show_speed_by_string == 1) {
		char szSpeed[32];
...
static ssize_t store_fan2_speed(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	unsigned long write_val = fan_speed_str_to_ul((char *)buf);
...
static ssize_t show_fan3_speed(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	unsigned int temp;
	if (flag_show_speed_by_string == 1) {
...
static ssize_t store_fan3_speed(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	unsigned long write_val = fan_speed_str_to_ul((char *)buf);
...
/* ---------- 4 steps attributes --------- */
static ssize_t show_fan_stop_speed(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", fan_stop_speed);
}
static ssize_t store_fan_stop_speed(struct device *dev,
...
static ssize_t show_fan_slow_speed(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", fan_slow_speed);
}
static ssize_t store_fan_slow_speed(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	unsigned long tmp_speed = simple_strtoul(buf, NULL, 10);
...
static ssize_t show_fan_fast_speed(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", fan_fast_speed);
}
static ssize_t store_fan_fast_speed(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	unsigned long tmp_speed = simple_strtoul(buf, NULL, 10);
...
/* --------- read mode attribute --------- */
static ssize_t show_show_speed_by_string(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	return sprintf(buf, "%d\n", flag_show_speed_by_string);
}
static ssize_t store_show_speed_by_string(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
...
DEVICE_ATTR(show_speed_by_string, 0644, show_show_speed_by_string,
	    store_show_speed_by_string);

/* --------- threshold attribute --------- */
static ssize_t show_fan_threshold(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", fan_threshold);
}
static ssize_t store_fan_threshold(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
...
static ssize_t show_sugi_fan_dev(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", sugi_fan_major);
}
...
static int show_hotplug_status(struct seq_file *m, void *v)
{
	...
	if (inl(ich10r_gpio_base_addr + hdd_info->HDD_GPIO_PRESENT_REG) &
	    hdd_info->HDD_PRESENT_BIT)
...
{
	return single_open(file, show_hotplug_status,
			   PDE_DATA(file_inode(file)));
}

#if HAVE_PROC_OPS
static struct proc_ops hotplug_status_fops = { .proc_open = open_hotplug_status,
					       .proc_read = seq_read,
					       .proc_lseek = seq_lseek,
					       .proc_release = single_release };
#else
...
static int show_power_control_status(struct seq_file *m, void *v)
{
	...
	if (inl(ich10r_gpio_base_addr + hdd_info->HDD_GPIO_POWER_REG) &
	    hdd_info->HDD_POWER_BIT)
		seq_printf(m, "on\n");
	else
		seq_printf(m, "off\n");
...
{
	return single_open(file, show_power_control_status,
			   PDE_DATA(file_inode(file)));
}

/* ----- set HDD power control status ---- */
static ssize_t store_power_control_status(struct file *file, const char *buffer,
					  size_t count, loff_t *offset)
{
	int tmpgpioval;
	struct sugi_hdd_info_st *hdd_info =
		(struct sugi_hdd_info_st *)PDE_DATA(file_inode(file));
	...
	if (strncmp(buffer, "off", strlen("off")) == 0) {
		if (tmpgpioval & hdd_info->HDD_POWER_BIT) {
			...
			outl(tmpgpioval, ich10r_gpio_base_addr +
						 hdd_info->HDD_GPIO_POWER_REG);
			return count;
		}
	}
...
static void SataHotplugPollingUpdatePinstat(void)
{
	...
	for (i = 0; i < MAX_SUPPORTED_DISKS; i++) {
		sata_hotplug_data[i].presentpinstat =
			(inl(ich10r_gpio_base_addr +
			     sugi_hdd_info[i].HDD_GPIO_PRESENT_REG) &
			 sugi_hdd_info[i].HDD_PRESENT_BIT) ?
				SATA_STAT_UNPLUGGED :
				SATA_STAT_PLUGGED;
	}
}
...
static void SataHotplugPolling(struct timer_list *__unused)
{
	...
	for (i = 0; i < MAX_SUPPORTED_DISKS; i++) {
		if (sata_hotplug_data[i].prevplugstat ==
		    sata_hotplug_data[i].presentpinstat) {
		...
		if (sata_hotplug_data[i].loops == 0) {
			if (sata_hotplug_data[i].presentpinstat ==
			    SATA_STAT_PLUGGED)
			...
			sata_hotplug_data[i].loops = SATA_POL_LOOPS;
			sata_hotplug_data[i].prevplugstat =
				sata_hotplug_data[i].presentpinstat;
		}
	}
 ...
 * ---------------------------------------------------------------------------------------------- */
static unsigned char checksum_caculation(unsigned char wol, unsigned char acp,
					 unsigned char lan)
{
...
static int smi_write(void)
{
	...
	for (smi_count = 0; smi_count < 3; smi_count++) {
		...
		for (read_count = 0; read_count < 10; read_count++) {
			...
			if (inb(CMOS_DATA_PORT) == 0x40 ||
			    inb(CMOS_DATA_PORT) == 0xEF ||
			    inb(CMOS_DATA_PORT) == 0xFF)
...
static ssize_t show_wol_setting(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int ret;
...
static ssize_t store_wol_setting(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t count)
{
	int ret;
	...
	if ((len == 3) && (strncmp(buf, "on", strlen("on")) == 0)) {
		...
		if (wol_setting_value == WOL_ENABLE) {
			checksum_caculated =
				checksum_caculation(wol_setting_value,
						    power_setting_value,
						    lanboot_setting_value);
			outb(CMOS_CHECKSUM_OFFSET, CMOS_INDEX_PORT);
...
static ssize_t show_power_setting(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	int ret;
...
static ssize_t store_power_setting(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	int ret;
	...
	if (strncmp(buf, "on", strlen("on")) == 0) {
		...
		if (bios_ver < 209) {
			ret = pci_read_config_word(pdev, GEN_PMCON_3,
						   &afterg3_en);
			afterg3_en &= (~0x0001);
			ret = pci_write_config_word(pdev, GEN_PMCON_3,
						    afterg3_en);
		}
...
static ssize_t show_lanboot_setting(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	int ret;
...
static ssize_t store_lanboot_setting(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	int ret;
...
/* ------- device number attribute ------- */
static ssize_t show_sugi_cmos_dev(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", sugi_cmos_major);
}
			      ...
			      char *buf)
{
	...
	return sprintf(
		...
		rv[0], rv[1], rv[2], rv[3], rv[4], rv[5], rv[6], rv[7], rv[8],
		rv[9], rv[10], rv[11], rv[12], rv[13], rv[14], rv[15], rv[16],
		rv[17], rv[18], rv[19], rv[20], rv[21], rv[22], rv[23], rv[24],
		rv[25], rv[26], rv[27], rv[28], rv[29], rv[30], rv[31]);
}

static ssize_t store_device_id(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
	int i;
...
/* --------- checksum attribute ---------- */
static ssize_t show_checksum(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	int i;
	unsigned int rv = 0;
...
static ssize_t store_checksum(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	int i, j, tmp, len = 0;
...
/* ------- device number attribute ------- */
static ssize_t show_sugi_boardid_dev(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", sugi_boardid_major);
}
...
static int init_proc_files(void)
{
	...
	for (i = 0; i < MAX_SUPPORTED_BUTTONS; i++) {
		switch_file[i] =
			proc_create_data(button_info[i].name, 0444, switch_dir,
					 &button_status_fops, &button_info[i]);
	}
	...
	for (i = 0; i < MAX_SUPPORTED_DISKS; i++) {
		...
		hotplug_sata_file[i] = proc_create_data(buf, 0444, hotplug_dir,
							&hotplug_status_fops,
							&sugi_hdd_info[i]);
	}
	...
	for (i = 0; i < MAX_SUPPORTED_DISKS; i++) {
		...
		power_control_hdd_file[i] =
			proc_create_data(buf, 0644, power_control_dir,
					 &power_control_status_fops,
					 &sugi_hdd_info[i]);
	}
...
static int init_sysfs_files(void)
{
	...
	boardid_dev =
		device_create(sugi_cls, NULL, sugit_boardid, NULL, "board");
...
static int bios_version_check(void)
{
	...
	for (i = 0; i <= 0xFFF0; i += 0x01) {
		...
		if (strncmp(smbios_sm, "_SM_", strlen("_SM_")) == 0) {
			...
			if (strncmp(smbios_dmi, "_DMI_", strlen("_DMI_")) ==
			    0) {
	...
	for (i = 0; i <= 0xFF; i += 0x01) {
		...
		if (ioread8(smbios_base) == 0x33) {
			if (strncmp(smbios_base, "31SUG", strlen("31SUG")) ==
			    0) {
				smbios_ver =
					ioremap(smbios_base_addr + i + 0x05, 1);
...
void SetBTNMBPnP(void)
{
	...
	it87_gpioset3_base_addr =
		(base_msb * 256 + base_lsb) + GPIO_SET3_OFFSET;
...
static int ich10r_setting(void)
{
	...
	outl((inl(ich10r_gpio_base_addr + GP_USE_SEL) | 0xE0000000),
	     ich10r_gpio_base_addr + GP_USE_SEL);

	/* set GPIO pin 29, 30 ,31 as input */
	outl((inl(ich10r_gpio_base_addr + GP_IO_SEL) | 0xE0000000),
	     ich10r_gpio_base_addr + GP_IO_SEL);

	/* get board ID relative GPIO pins status */
	boardid_num = (inl(ich10r_gpio_base_addr + GP_LVL) & BOARD_ID_MASK) >>
		      BOARD_ID_OFFSET;

	/* HDD control driver */
	/* set relative GPIO pins as GPIO function */
	outl((inl(ich10r_gpio_base_addr + GP_USE_SEL) | 0x0002A6FD),
	     ich10r_gpio_base_addr + GP_USE_SEL);
	outl((inl(ich10r_gpio_base_addr + GP_USE_SEL2) | 0x01000007),
	     ich10r_gpio_base_addr + GP_USE_SEL2);

	/* set hotplug relative GPIO pins as input */
	outl((inl(ich10r_gpio_base_addr + GP_IO_SEL) | 0x0000A63C),
	     ich10r_gpio_base_addr + GP_IO_SEL);

	/* set power control relative GPIO pins as outout */
	outl((inl(ich10r_gpio_base_addr + GP_IO_SEL) & 0xFFFDFF3D),
	     ich10r_gpio_base_addr + GP_IO_SEL);
	outl((inl(ich10r_gpio_base_addr + GP_IO_SEL2) & 0xFEFFFFF8),
	     ich10r_gpio_base_addr + GP_IO_SEL2);
...
static int __init sugi_platform_init(void)
{
	...
	/* request IRQ3 */
	ret = request_irq(3, sugi_interrupt, IRQF_SHARED, DRVNAME,
			  sugi_interrupt);
	if (ret) {
